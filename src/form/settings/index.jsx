...
function ChangeAvatarForm({ onClose }) {
    ...
    const OnFormSubmit = async (formData) => {
        const user = await fetchData(`/profiles/${name}/media`, { method: 'Put', body: JSON.stringify(formData) });;
        if (user.isError) {
        ...
        <>
            ...
            {errorMessage && <p style={{ color: 'var(--color-red)' }}>{errorMessage}</p>}
                ...
                <div className="d-grid">
                    <LoadingButton buttonText="Save Changes" type="submit" isValid={isValid} Loading={isLoading} />
                </div>
            </form>
        </>
    );
